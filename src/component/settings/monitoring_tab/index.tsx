...
const MonitoringTab: React.FC<MonitoringTabProps> = ({ userRole }) => {
	const [settings, setSettings] = useState<EventSetting[]>([
		...
		},
		{
			...
			description:
				'Администратор обновил матрицу компетенций для вашей должности',
			enabled: false,
		},
	]);
	...
	const allSettings =
		userRole === 'manager' ? [...settings, ...managerSettings] : settings;
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.description}>
				<p>
					Настройте отображение информационных событий на главном экране в блоке
					«Последние события».
				</p>
			</div>
				...
				<div className={styles.eventsPreview}>
					<div className={styles.eventItem}>
						...
						<span className={styles.eventText}>
							➕ Назначен курс "Продвинутый JavaScript"
						</span>
					</div>
					<div className={styles.eventItem}>
						<span className={styles.eventDate}>Вчера</span>
						<span className={styles.eventText}>
							📊 Оценка компетенции "React" изменена на 3
						</span>
					</div>
					<div className={styles.eventItem}>
						<span className={styles.eventDate}>15.03</span>
						<span className={styles.eventText}>
							📝 Обновлены требования к должности "Ведущий разработчик"
						</span>
					</div>
				</div>
			</div>
